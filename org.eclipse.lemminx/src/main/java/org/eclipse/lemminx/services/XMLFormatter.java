...
class XMLFormatter {
	...
	private static final boolean NEW_FORMATTER = true;
	...
	public List<? extends TextEdit> format(TextDocument textDocument, Range range, SharedSettings sharedSettings) {
		try {
			if (NEW_FORMATTER) {
				XMLFormatterDocumentNew formatterDocument = new XMLFormatterDocumentNew(textDocument, range, sharedSettings,
